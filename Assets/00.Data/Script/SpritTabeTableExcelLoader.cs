...
[System.Serializable]
public struct SpritTabeTableExcel
{
	public int No;
...
[CreateAssetMenu(fileName="SpritTabeTableLoader", menuName= "Scriptable Object/SpritTabeTableLoader")]
public class SpritTabeTableExcelLoader :ScriptableObject
{
	[SerializeField] string filepath =@"Assets\00.Data\Txt\SpritTabeTable.txt";
	public List<SpritTabeTableExcel> DataList;

	private SpritTabeTableExcel Read(string line)
	{
		line = line.TrimStart('\n');

		SpritTabeTableExcel data = new SpritTabeTableExcel();
		int idx =0;
	...
	{
		DataList=new List<SpritTabeTableExcel>();
		...
		foreach (var item in strs)
		{
			if(item.Length<2)
				continue;
			SpritTabeTableExcel data = Read(item);
			DataList.Add(data);
		}
	}
}
