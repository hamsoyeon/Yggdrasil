...
namespace test_client_unity
{
    public class StageState : State
    {
                ...
                case (int)StageMgr.SUB_PROTOCOL.STAGE_RESULT:
                    {
                        switch (detail_prototocol)
                        {
                            case (int)StageMgr.SERVER_DETAIL_PROTOCOL.ENTER_INGAME:
                                ...
                                break;
                            case (int)StageMgr.SERVER_DETAIL_PROTOCOL.STAGE_EXIT_SUCCESS:
                                teve.eve = (int)RECV_EVENT.STAGE_EXIT;
                                break;
                        }
                    }
                    break;
                ...
                case (int)RECV_EVENT.SELECTED_CHAR:
                    {
                        ...
                        StageMgr.Instance.Selected_Char(charType, flag);
                    }
                    break;
                case (int)RECV_EVENT.CHAT_FILED:
                    {
                        ...
                        GameObject stage = Change_UI_Mgr.Instance.GetUI(Change_UI_Mgr.UI_TYPE.STAGE);
                        stage.transform.GetComponentInChildren<ChatPanel>().UpdateField(msg);
                        Debug.Log(msg);
                    }
                    break;
                case (int)RECV_EVENT.STAGE_EXIT:
                    {
                        string[] name = new string[3];
                        int[] charType = new int[3] { -1, -1, -1 };
                        int arrNum = new int();
                        StageMgr.Instance.Unpackpacket(_eve.buf, ref arrNum, ref name, ref charType);
                        StageMgr.Instance.ReSetStage(arrNum, name, charType);
                    }
                    break;
            }
        }
        ...
        public override void QuitProgram(t_Eve _eve)
        {
            ...
            ProtocolMgr.Instance.AddDetailProtocol(ref protocol, (uint)StageMgr.DETAIL_PROTOCOL.STAGE_EXIT);
